...
- **Explorer Icon Reliability Fix:** Rebuilt the Win32 `.ico` resource in a compiler-safe format so large/tile Explorer views use the custom icon instead of falling back to `.NET`.
...
- **Daily Double Modal Layout Fix:** Corrected clue modal content flow so the judge panel and answer controls no longer get pushed to the bottom when the Daily Double banner is shown.
